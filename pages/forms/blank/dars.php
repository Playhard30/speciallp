...
$pdf->Cell(20, 4, $_SESSION['active_acad'], 'B', 0, 'C');
$pdf->Cell(2);
...
if ($_SESSION['active_sem'] == 'First Semester') {
    $pdf->SetFont('Arial', '', 8);
    $pdf->Cell(120, 4, '', 0, 0);
...
} elseif ($_SESSION['active_sem'] == 'Second Semester') {
    $pdf->SetFont('Arial', '', 8);
    $pdf->Cell(120, 4, '', 0, 0);
    $pdf->Cell(3, 3, '', 1, 0, 'C');
    $pdf->Cell(0, 3, '1st Semester', 0, 1);
    ...
    $pdf->Cell(3, 3, '', 1, 0, 'C');
    $pdf->Cell(0, 3, 'Summer', 0, 1);
} elseif ($_SESSION['active_sem'] == 'Summer') {
    $pdf->SetFont('Arial', '', 8);
    $pdf->Cell(120, 4, '', 0, 0);
    $pdf->Cell(3, 3, '', 1, 0, 'C');
    $pdf->Cell(0, 3, '1st Semester', 0, 1);
...
if ($row['curri'] == "New Curri") {
    ...
    WHERE tbl_enrolled_subjects.stud_id = '$_GET[stud_id]' AND tbl_enrolled_subjects.acad_year = '$_SESSION[active_acad]' AND tbl_enrolled_subjects.semester = '$_SESSION[active_sem]' ORDER BY tbl_enrolled_subjects.subj_id") or die(mysqli_error($db));
    $pdf->SetFont('Arial', '', '9');
    $y = $pdf->Gety();
    $xy = 3.5;
    $i = 1;
    ...
    $pdf->SetXY(13, 52);
    $sql = mysqli_query($db, "SELECT *,CONCAT(tbl_faculties_staff.faculty_lastname, ', ', tbl_faculties_staff.faculty_firstname, ' ', tbl_faculties_staff.faculty_middlename)  as fullname, tbl_students.stud_id,tbl_subjects.subj_id,tbl_schedules_old.class_id,tbl_faculties_staff.faculty_id FROM tbl_enrolled_subjects
    LEFT JOIN tbl_students ON tbl_students.stud_id = tbl_enrolled_subjects.stud_id
...
// $p = mysqli_fetch_array(mysqli_query($db,"SELECT count(subj_id) as subj FROM tbl_enrolled_subjects where stud_id = '$_GET[stud_id]' AND tbl_enrolled_subjects.acad_year = '$_SESSION[active_acad]' AND tbl_enrolled_subjects.semester = '$_SESSION[active_sem]'")) or die(mysqli_error($db)); 
...
if ($row['curri'] == "New Curri") {
    ...
    $pdf->SetXY(13, 48.5);
    $sql2 = mysqli_query($db, "SELECT *,CONCAT(tbl_faculties_staff.faculty_lastname, ', ', tbl_faculties_staff.faculty_firstname, ' ', tbl_faculties_staff.faculty_middlename)  as fullname, tbl_students.stud_id,tbl_subjects.subj_id,tbl_schedules_old.class_id,tbl_faculties_staff.faculty_id FROM tbl_enrolled_subjects
    LEFT JOIN tbl_students ON tbl_students.stud_id = tbl_enrolled_subjects.stud_id
    LEFT JOIN tbl_subjects ON tbl_subjects.subj_id = tbl_enrolled_subjects.subj_id
    LEFT JOIN tbl_schedules_old ON tbl_schedules_old.class_id = tbl_enrolled_subjects.class_id
    LEFT JOIN tbl_faculties_staff ON tbl_faculties_staff.faculty_id = tbl_schedules_old.faculty_id
    ...
    while ($row2 = mysqli_fetch_array($sql2)) {
        ...
        $pdf->Cell(34, 3, $row2['subj_code'], 0, 0, 'C');
        $pdf->Cell(15, 3, $row2['day'], 0, 0, 'C');
        $pdf->Cell(18, 3, $row2['time'], 0, 0, 'C');
        $pdf->Cell(12, 3, $row2['room'], 0, 0, 'C');
        $xy += 3;
        $i++;
    }
}
...
$pdf->Cell(5, 3, '', 0, 0);
$pdf->Cell(20, 3, $_SESSION['active_acad'], 'B', 0);
...
if ($row['curri'] == "New Curri") {
    ...
    $sql3 = mysqli_query($db, "SELECT *,CONCAT(tbl_faculties_staff.faculty_lastname, ', ', tbl_faculties_staff.faculty_firstname, ' ', tbl_faculties_staff.faculty_middlename)  as fullname, tbl_students.stud_id,tbl_subjects.subj_id,tbl_schedules_old.class_id,tbl_faculties_staff.faculty_id FROM tbl_enrolled_subjects
    LEFT JOIN tbl_students ON tbl_students.stud_id = tbl_enrolled_subjects.stud_id
    LEFT JOIN tbl_subjects ON tbl_subjects.subj_id = tbl_enrolled_subjects.subj_id
    LEFT JOIN tbl_schedules_old ON tbl_schedules_old.class_id = tbl_enrolled_subjects.class_id
    LEFT JOIN tbl_faculties_staff ON tbl_faculties_staff.faculty_id = tbl_schedules_old.faculty_id
...
$pdf->Cell(5, 3, '', 0, 0);
$pdf->Cell(20, 3, $_SESSION['active_acad'], 'B', 0, 'C');
$pdf->Cell(5, 3, 'Date:', 0, 0);
$pdf->Cell(5, 3, '', 0, 0);
