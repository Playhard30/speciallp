...
if (isset($_POST['add'])) {
    ...
    if (!empty($_POST['day']) and !empty($_POST['time']) and !empty($_POST['room'])) {
        ...
        if ($count == 0) {
            // insert Data
            $query = $db->query("INSERT INTO tbl_schedules (class_code, subj_id, faculty_id, day, time, room, section, acad_year, semester, special_tut) VALUES ('$subj_code', '$subj_id', '$instructor', '$day' , '$time', '$room', '$section', '$_SESSION[AC]', '$_SESSION[S]', '$special_tut')") or die($db->error);
            $_SESSION['SASched'] = true;
            unset($_COOKIE['instructor']);
            header("location: ../offer.BSHM.php?eay=" . $_SESSION['back_eay'] . "&CID=" . $_SESSION['back_CID']);
        } else {
...
if (isset($_POST['add_petitioned'])) {
    ...
    // required fields
    if (!empty($_POST['day']) and !empty($_POST['time']) and !empty($_POST['room']) && !empty($_COOKIE['$subj_id'])) {
        // Invalid same content
        $check = $db->query("SELECT * FROM tbl_schedules WHERE class_code = '$subj_code' AND subj_id = '$subj_id' AND faculty_id = '$instructor' AND day = '$day' AND time = '$time' AND room = '$room' AND section = '$section' AND acad_year = '$_SESSION[AC]' AND semester = '$_SESSION[S]' AND special_tut = '$special_tut'") or die($db->error);
        $count = $check->num_rows;
        if ($count == 0) {
            // insert Data
            $query = $db->query("INSERT INTO tbl_schedules (class_code, subj_id, faculty_id, day, time, room, section, acad_year, semester) VALUES ('$subj_code', '$subj_id', '$instructor', '$day' , '$time', '$room', '$section', '$_SESSION[AC]', '$_SESSION[S]')") or die($db->error);
            $_SESSION['SASched'] = true;
            unset($_COOKIE['instructor']);
            header("location: ../offer.BSHM.php?eay=" . $_SESSION['back_eay'] . "&CID=" . $_SESSION['back_CID']);
        } else {
